...
def get_attribute(comp, name):
    try:
        return comp.FindAttribute(name)
    except Exception:
        return None
...
def set_component_value(comp, value, src_attr=None):
    ...
    if src_attr is not None:
        try:
            dst_attr = get_attribute(comp, src_attr.Name)
        except Exception:
            dst_attr = None
    if dst_attr is None:
        dst_attr = get_attribute(comp, "Value")
    if dst_attr is None:
        dst_attr = get_attribute(comp, "VALUE")
    if dst_attr is None:
        try:
            ...
            return False
    ok = set_attribute_value(dst_attr, value)
    if ok and src_attr is not None:
        try:
...
def main():
    ...
    try:
        ...
        copy_components(src_view, dst_block)
        copy_nets(src_view, dst_block)
    finally:
        app.SetRedraw(True)
    ...
    print(f"{dst_sheet} created and copied from {schematic_name}:{src_sheet}.")
